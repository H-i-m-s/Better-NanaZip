...
这些脚本用于在当前电脑上创建和使用 SSS 的开发证书，让自建 NanaZip MSIX/MSIXBundle 可以本机安装测试。
...
```text
E:\NanaZip\NanaZipPackage\Package.appxmanifest
```
...
- `Build-SSSNanaZipPackage.ps1`：调用 NanaZip 完整构建，并可对生成包批量签名。
...
公钥：E:\NanaZip\.local\SSSDevSigning\SSS-NanaZip-Development.cer
```

`.local/` 已写入本仓库的本地 exclude，不会进入普通 Git 状态；`.pfx` 也被根 `.gitignore` 忽略。仍然不要主动 `git add -f` 这些文件。

## 使用顺序

### 1. 创建证书

证书已经在当前电脑创建完成。以后如果需要重新创建，推荐使用 store-only 模式：

```powershell
cd E:\NanaZip
.\tools\SSSDevSigning\New-SSSDevCertificate.ps1 -StoreOnly
```
...
如果以后要给 GitHub Actions 或其他构建机使用，再运行不带 `-StoreOnly` 的模式，脚本会提示设置 PFX 密码。PFX 包含私钥，不能提交或分享。

### 2. 信任证书

当前电脑已经完成这一步。以后重新创建证书后执行：

```powershell
.\tools\SSSDevSigning\Install-SSSDevCertificate.ps1
```
...
```powershell
cd E:\NanaZip
.\tools\SSSDevSigning\Build-SSSNanaZipPackage.ps1 -SignAllMsix
```

如果已经构建过，只想对现有产物签名：

```powershell
.\tools\SSSDevSigning\Build-SSSNanaZipPackage.ps1 -SkipBuild -SignAllMsix
```
...
```powershell
.\tools\SSSDevSigning\Sign-SSSNanaZipPackage.ps1 `
  -PackagePath "E:\NanaZip\Output\Binaries\AppPackages\你的包.msixbundle"
```
...
```powershell
.\tools\SSSDevSigning\Verify-SSSNanaZipPackage.ps1 `
  -PackagePath "E:\NanaZip\Output\Binaries\AppPackages\你的包.msixbundle"
```
...
```powershell
.\tools\SSSDevSigning\Install-SSSNanaZipPackage.ps1 `
  -PackagePath "E:\NanaZip\Output\Binaries\AppPackages\你的包.msixbundle"
```
...
```powershell
.\tools\SSSDevSigning\Install-SSSNanaZipPackage.ps1 `
  -PackagePath "E:\NanaZip\Output\Binaries\你的包.msixbundle" `
  -ForceUpdate
```
